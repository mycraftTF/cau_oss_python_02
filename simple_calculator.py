...
while True:
    ...
    if op == "end":
        ...
        num1, ret, num2 = arithmetic_ops(add) # 정의된 함수 사용
    elif op == "*":
        num1, ret, num2 = arithmetic_ops(lambda x,y: x*y) # 익명함수(lambda) 사용
                    # [fill this area] 위의 코드를 참고하여 -, /, %에 대한 내용 구현
    elif op == '-': 
        num1, ret, num2 = arithmetic_ops(sub) 
    elif op == '/':
        num1, ret, num2 = arithmetic_ops(lambda x,y: x/y)
    elif op == '%':
        num1, ret, num2 = arithmetic_ops(lambda x,y: x%y)
